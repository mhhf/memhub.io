...
This can be solved by a **type** system: An organization can be restricted to an underlaying structure: a set of types it will accepts as valid candidates denoted as $\\mathcal{L}$.
...
In our example Dave couldn't have proposed ![](cat.svg) if the organization had the set of types or "**language**" $\\mathcal{L}_1=\\lbrace String \\rbrace$. However, it would be a valid candidate with the language $\\mathcal{L}_2=\\lbrace String, Image\\rbrace$. The proposed candidate has to satisfy just one of the **alternatives** in the set to in order to be valid.
Another form to write alternatives is $\\mathcal{L}_2=(String | Image)$. For a language $\\mathcal{L}$ we also adopt the colon notation to state that a candidate matches *one* of the types in the typeset: $"cat\\,corp":\\mathcal{L}_2$ is valid.

If an organization would want to find a match composed out of a name and a logo, they could have chosen the language $\\mathcal{L}_3=\\lbrace (String, Image) \\rbrace$. A valid candidate would have to contain a String and an Image:
...
md_222 =`
Note here that the type $(String, Image)$ is a **composition** out of the type $String$ and the type $Image$, so composing types is a valid operation to come up with another type. We can write composition with the dot notation: $String\\cdot Image$.

Also we want allow the Kleene operator for language description:
